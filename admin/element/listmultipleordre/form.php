         ...
         if($_GET["mode"]=="ajout"||$_GET["mode"]=="modif"){
            ...
            print "<select name=\"".$tabelem[0]."\" class=\"selectstyle\" size=\"6\" multiple style=\"min-width:250px\">>";
             while($tbl_resultselect = fetch ($resultatselect)){
	          ...
	          print "</select></td>";
            print "<td><input type=\"button\" name=\"up\" value=\"Plus haut\" onclick=\"upselect(this.form.elements['".$tabelem[0]."'])\"><br><input type=\"button\" name=\"up\" value=\"Plus bas\" onclick=\"downselect(this.form.elements['".$tabelem[0]."'])\"></td></tr></table>";
            //</div>
         }else{
         	$resultatselect=query($$querystring[1]);
            while($tbl_resultselect = fetch ($resultatselect)){
