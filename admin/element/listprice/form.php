...
$querystring="select* from prix where content_id=".$tbl_result["content_id"]." and supprimer=0";
if($_GET["mode"]=="ajout"||$_GET["mode"]=="modif"){
  ?>
  <div style="border:1px solid black;width:500px;height:200px">
  <iframe frameborder="0" border="0" src="../element/listprice/insertprice.php?nomobj=<?=$tabelem[0]?>" width="500" height="200" scrolling="no" name="iframesave"></iframe>
  </div>
  <br>
  <span id="listprice">
  <?
  ...
  while($tbl_resultselect = fetch ($resultatselect)){
      ...
      <tr><td>Prix :</td><td><input type="text" name="montant<?=$tbl_resultselect["prix_id"]?>" value="<?=$tbl_resultselect["montant"]?>" /></td></tr>
      <tr><td>Prix remisé (0 pas de remise) :</td><td><input type="text" name="montantremise<?=$tbl_resultselect["prix_id"]?>" value="<?=$tbl_resultselect["montantremise"]?>" /></td></tr>
      <tr><td>Quantité :</td><td><input type="text" name="quantite<?=$tbl_resultselect["prix_id"]?>" value="<?=$tbl_resultselect["quantite"]?>" /></td></tr>
      <tr><td>Référence :</td><td><input type="text" name="ref<?=$tbl_resultselect["prix_id"]?>" value="<?=$tbl_resultselect["ref"]?>" /></td></tr>
      <tr><td colspan="2" align="right"><input type="button" name="modifier" value="<?=$trad["Modifier"]?>" onclick="savemodif(this,<?=$tbl_resultselect["prix_id"]?>)"/><input type="button" name="supprimer" value="<?=$trad["Supprimer"]?>" onclick="document.getElementById('table_prix_<?=$tbl_resultselect["prix_id"]?>').style.display='none';document.getElementById('prix_<?=$tbl_resultselect["prix_id"]?>').value='1'"/></td></tr></table>
      <?
  }
  }
  ?>
  ...
  while($tbl_resultselect = fetch ($resultatselect)){
      ...
      <tr><td>Prix remisé (0 pas de remise) :</td><td><?=$tbl_resultselect["montantremise"]?></td></tr>
