...
*/
  ...
  */
  if ( GetE('txtWidth').value.length == 0 ){
      GetE('txtWidth').value=400;
  }
  if ( GetE('txtHeight').value.length == 0 ){
    if(GetExt(GetE('txtURL').value)=="flv"){
      GetE('txtHeight').value=300;
    }else{
...
Media.prototype.getInnerHTML = function (objectId){
	...
	//s+= '</p>\n';
  */
  vars='bufferlength=3&largecontrols=true&logo='+ this.wmurl;
  var colorChoice1 = this.toolcolor
	if (colorChoice1.length > 0) {
		colorChoice1 = colorChoice1.replace("#","0x")
		vars+= '&backcolor=' + colorChoice1;
	}
	// SET THE COLOR OF THE TOOLBARS TEXT AND BUTTONS
	var colorChoice2 = this.tooltcolor
	if (colorChoice2.length > 0) {
		colorChoice2 = colorChoice2.replace("#","0x")
		vars+= '&frontcolor=' + colorChoice2;
	}
	//SET COLOR OF ROLLOVER TEXT AND BUTTONS
	var colorChoice3 = this.tooltrcolor
	if (colorChoice3.length > 0) {
		colorChoice3 = colorChoice3.replace("#","0x")
		vars+= '&lightcolor=' + colorChoice3;
	}
	//SET COLOR OF BACKGROUND
	var colorChoice4 = this.bgcolor
	if (colorChoice4.length > 0) {
		colorChoice4 = colorChoice4.replace("#","0x")
		vars+= '&screencolor=' + colorChoice4;
	}
  vars+='&showdownload=' + this.downloadable + '&link=' + this.url + '&showdigits=' + this.displayDigits + '&shownavigation=' + this.displayNavigation;
  vars+='&file=' + this.url + '&repeat=' + this.loop + '&image=' + this.iurl+'&repeat='+this.loop+'&shuffle=false&width=' + thisWidth + '&height=' + thisHeight + '&autostart=' + this.play;
	/*
  s+= '<object id="player'+randomnumber+'" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" name="player'+randomnumber+'" width="'+thisWidth+'" height="'+thisHeight+'">';
  s+= '<param name="movie" value="'+oEditor.FCKConfig.playerUrl+'" />';
  ...
  */
  ...
  s+= 'id="player2'+randomnumber+'" ';
  s+= 'name="player2'+randomnumber+'" ';
  s+= 'src="'+oEditor.FCKConfig.playerUrl+'" ';
  s+= 'width="'+thisWidth+'" ';
  s+= 'height="'+thisHeight+'" ';
