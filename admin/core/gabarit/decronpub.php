...
if($droit){
  ...
  if($_POST["mode"]=="save"){
  $sql="update ".__racinebd__."arbre set datedepublication='".datetimebdd($_POST["datepublication"])."' where arbre_id=".$_GET["arbre_id"];
  $link=query($sql);
  ?>
  ...
  <center>
    <?=$trad["Date de dépublication"]?><br><input type="texte" id="datepublication" name="datepublication" value="<?=affichedatetime($tbl["datedepublication"])?>"/><img src="../images/datepopup.gif" id="triggerdatepublication" style="cursor:pointer; cursor:hand;"><br><input type="submit" name="valider" value="<?=$trad["valider"]?>" />
  </center>
