...
<?cache_require("rapport-menu.php")?>
  <div class="main_resize backgroundbloc ajustmargin">
    <div class="resize rapport_flotte">
      <h1 class="bcktitle ajust_title ">Rapport de flotte journalier</h1>
      <form class="form_rapport_flotte" action="<?=urlp($_GET["arbre"])?>" method="post">
      <input type="hidden" value="flotte" name="template">
      ...
      <span class="info_rapportkm">
        <p>Conduite : <span> <?=$totalconduite?> </span> min</p>
        <p>Arrêt : <span> <?=secondsToTime($totalarret)?> </span></p>
        <!-- <p>Attente : <span> 00 </span> min </p> -->
        <p>Distance : <span> <?=$totaldistance?> </span>Km</p>
        <a href="javascript:window.print()"><img src="<?=__racineweb__?>/tpl/img/imprimer_ico.png"></a>
        <?=showexport("flotte",array("Nom","Début","Fin","Ampl.","Conduite","Arrêt","Distance","Vit.max.","Conso."),array("nomvehicule","mintime","maxtime","amplitude","datediff","arret","km","vitesse","conso"))?>
      </span>
    </div>
  </div>
  <div class="bottom_content">
    <table class="tableau_content tableau_comtperapport">
      ...
      <tr>
        ...
        <td>3F ASSISTANCE</td>
        -->
        <td><?=afficheheure($tbl_list_vehicule[$i]["mintime"])?></td>
        <td><?=afficheheure($tbl_list_vehicule[$i]["maxtime"])?></td>
        ...
        <?}else{?>
        <td colspan="5">Journée en cours</td>
        <?}?>
      </tr>
      <?}?>
    </table>
    ...
    <script src="<?=__racineweb__?>/tpl/js/rapport-flotte.js"></script>
